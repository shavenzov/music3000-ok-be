<?php
   ...
   class Acapella extends SessionManager
   {
	  ...
	  private function getSearchQueryString( $params )
	  {
		...
		{
		  $whereStr .= "( autotune = '{$params->autotune}' )";
		  $queryParts[] = $whereStr;
		}
		...
		if ( isset( $params->duration ) )
		{
		   $whereStr .= "( duration <= {$params->duration} )";
		   $queryParts[] = $whereStr;
		}
	  ...
	  public function getSearchParams( $session_id, $params, $getGenres, $getGenders, $getTempos, $getKeys, $getStyles, $getAutoTunes  ) 
	  {
		...
		if ( $getGenres )
		 {
		   ...
		   {
		     $r = mysql_query( "select genre from {$this->acapella_table} group by genre" );
		   }
		   else
		   {
		     $r = mysql_query( "select genre from {$this->acapella_table}  where {$whereStr} group by genre" );
		   }
		...
		if ( $getGenders )
	    {
		   ...
		   if ( strlen( $whereStr ) == 0 )
		   {
		    $r = mysql_query( "select gender from {$this->acapella_table} group by gender" );
		   }
		 else
		 {
		    $r = mysql_query( "select gender from  {$this->acapella_table} where {$whereStr} group by gender" );
		 }
		...
		if ( $getStyles )
	    {
		   ...
		   if ( strlen( $whereStr ) == 0 )
		   {
		     $r = mysql_query( "select style from {$this->acapella_table} group by style" );
		   }
		   else
		   {
		     $r = mysql_query( "select style from {$this->acapella_table} where {$whereStr} group by style" );
		   }
		...
		if ( strlen( $whereStr ) == 0 )
		 {
		   $r = mysql_query( "select mkey from {$this->acapella_table} group by mkey" );
		 }
		 else
		 {
		    $r = mysql_query( "select mkey from {$this->acapella_table} where {$whereStr} group by mkey" );
		 }
		...
		if ( $getAutoTunes )
	    {
		   ...
		   if ( strlen( $whereStr ) == 0 )
		   {
		    $r = mysql_query( "select autotune from {$this->acapella_table} group by autotune" );
		   }
		 else
		 {
		    $r = mysql_query( "select autotune from {$this->acapella_table} where {$whereStr} group by autotune" );
		 }
	  ...
	  */
	  ...
	  public function getInfo( $session_id, $ids )
	  {
		 ...
		 $r = mysql_query( "select hash, name, author, type, tempo, duration, genre, gender, style, autotune, mkey from {$this->acapella_table} where {$where}" );
	  ...
	  public function search( $session_id, $params )
	  {
		...
		if ( strlen( $whereStr ) == 0 )
		 {
		   $r = mysql_query( "select count(*) as count from {$this->acapella_table}" );
		 }
		 else
		 {
		   $r = mysql_query( "select count(*) as count from {$this->acapella_table} where {$whereStr}" ); 
		 }
		...
		if ( strlen( $whereStr ) == 0 )
		 {
		   $r = mysql_query( "select hash, name, author, type, tempo, duration, genre, gender, style, autotune, mkey from {$this->acapella_table} order by {$params->orderBy} {$params->order} limit {$params->offset},{$params->limit}" );
		 }
		 else
		 {
		   $r = mysql_query( "select hash, name, author, type, tempo, duration, genre, gender, style, autotune, mkey from {$this->acapella_table} where {$whereStr} order by {$params->orderBy} {$params->order} limit {$params->offset},{$params->limit}" );
		 }
